...
<cfif structkeyexists(FORM,'update_menu') and FORM.recordID NEQ ''>
	<cfquery name="SaveUserActivity" datasource="warfightersupport">
		UPDATE tbl_ace_nav
...
<cfquery datasource="warfightersupport" name="get_navigations">
...
<cfif structKeyExists(URL, 'id') and URL.id neq ''>
	<cfquery datasource="warfightersupport" name="qGetNavigation">
		SELECT * FROM tbl_ace_nav  
		WHERE recordID  = <cfqueryparam value="#URL.id#" cfsqltype="cf_sql_integer">                    
	</cfquery>
...
<cfquery datasource="warfightersupport" name="get_navigations_parent">
	SELECT * FROM tbl_ace_nav   
	WHERE menuParent = 0                 
</cfquery>
				...
				<table id="navigation_table" class="table">
					...
					<tbody>
						<cfloop query="get_navigations">
							<cfquery datasource="warfightersupport" name="get_navigation_parent">
								SELECT * FROM tbl_ace_nav   
